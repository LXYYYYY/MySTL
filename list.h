...
namespace mystl
{
	...
	struct node_traits
	{
		typedef list_node_base<T>* bast_ptr;
	...
	struct list_node_base
	{
		...
		base_ptr prev; // 指向前一个节点
		bast_ptr next; // 指向下一个节点
		...
		base_ptr self()
		{
			return stataic_cast<base_ptr>(&*this);
		}
	};
	...
	// list的迭代器设计
	template <class T>
	struct list_iterator : public mystl::iterator<mystl:bidirectional_tag, T>
	{
		typedef	T									value_type;
		typedef T*									pointer;
		typedef	T&									reference;
		typedef typename node_traits<T>::base_ptr	base_ptr;
		typedef typename node_tarits<T>::node_ptr	node_ptr;
		typedef	list_iterator<T>					self;

		base_ptr node; // 迭代器内部的指针，指向list的节点
	...
	public:
		...
		typedef mystl::allocator<list_node<T>>				ndoe_allocator;
